...
class RasterDashboard(Dashboard):
    ...
    # ------------------------------------------------------------------------
    # initializeTimeStampsOptions -- as "initializeVariableOptions" in dashboard.py
    # ------------------------------------------------------------------------
    def initializeTimeStampsOptions(self):
        
        allOptions: list = []
        for key in self._datasetsVariables.keys():
            tstampsForThisCollection: list = self._datasetsData[key].time.values
    ...
    def generateRasterLayer(self, varOpt, timeOpt):
        ...
        rasterLayer = ds.hvplot.quadmesh(x='lon', y='lat', 
                                         geo=True, 
                                         cmap='jet',
                                         rasterize=True,
                                         frame_width=1200,
    ...
    def view(self):
        ...
        try:
            ...
            rasterPlot = pn.bind(self.generateRasterLayer, \
